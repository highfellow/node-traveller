...
Translation and string substitution wrapper module for node, using node-gettext but providing a cleaner and more javascript-ish interface.
...
* Usually, locale data will be kept in some kind of 'path/locale/file.[po|json]' structure, where path is either a filesystem path or an http path. Traveller also provides a neat interface for loading language files automatically by locale name.
